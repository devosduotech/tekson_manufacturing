"""Planner Exceptions with Severity"""
import frappe
...
def get_exceptions(planned_date=None):
    ...
    for wo in overdue_wo:
        ...
        overdue.append({"name": wo.name, "item": wo.production_item, "date": str(wo.planned_start_date)[:10], "days_overdue": days})
    ...
    blocked = [{"name": jc.name, "wo": jc.work_order, "op": jc.operation, "reason": jc.custom_blocked_by or jc.custom_readiness_status} for jc in blocked_jcs]
    ...
    return {
        "summary": {"overdue": len(overdue_wo), "blocked": len(blocked_jcs), "material_short": len(short_jcs), "dependency_wait": len(dep_jcs)},
        "severity": severity,
        "overdue": overdue, "blocked": blocked, "material_short": material_short, "dependency_wait": dependency_wait
    }
