frappe.pages['material-planning'].on_page_load = function(wrapper) {
    ...
    let html = `<div style="padding:30px; max-width:800px;">
        <h3>Daily Material Planning</h3><hr>
        <div class="row">
            <div class="col-sm-6"><label>Production Plan</label><select id="pp_select" class="form-control"></select></div>
            <div class="col-sm-3"><label>Planned Start Date</label><input type="date" id="plan_date" class="form-control"></div>
            <div class="col-sm-3"><br><button id="gen_btn" class="btn btn-primary">Generate MRs</button></div>
        </div>
    ...
    page.body.find('#gen_btn').on('click',function(){
        let pp=page.body.find('#pp_select').val(), dt=page.body.find('#plan_date').val();
        if(!pp){frappe.msgprint('Select a Production Plan');return;}
        let btn=page.body.find('#gen_btn');btn.prop('disabled',true).text('Generating...');
        frappe.call({method:'tekson_manufacturing.planning.material_planning_service.generate_daily_material_requests',args:{production_plan:pp,planned_date:dt},callback:function(r){
            ...
            let res=r.message||{}, area=page.body.find('#result_area').empty();
            if(res.created_mrs&&res.created_mrs.length>0){
                ...
                (res.details||[]).forEach(function(d){area.append('<div class="alert alert-info" style="margin:3px 0"><a href="/app/material-request/'+d.name+'"><b>'+d.name+'</b></a> — '+d.department_wip+' ('+d.items+' items)</div>');});
